...
import ArbolDeExpansionMinima.Prim;
import grafo.Grafo;
 ...
 * @author erika
 */
public class PnlPrim extends javax.swing.JPanel {
    private Grafo grafo;
    private Map<String, Point> posiciones;
    private Set<String> nodosEnMST = new HashSet<>();
    private Set<String> aristasEnMST = new HashSet<>();
    private Set<String> aristasEvaluadas = new HashSet<>();
    private boolean procesoCompleto = false;
    private double pesoTotal = 0;
    private String ciudadInicial;

    /**
     * Creates new form PnlPrim
     */
    public PnlPrim(String ciudadInicial) {
        ...
        new Thread(() -> {
            try {
                Thread.sleep(500);
                Prim.setPanelVisualizacion(this);
                Prim.encontrarMST(ciudadInicial);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }).start();
    ...
    /**
     * Agregga los nodos al MST
     * @param nodo 
     */
    public void agregarNodoAMST(String nodo) {
        nodosEnMST.add(nodo);
        repaint();
     ...
     */
    public void agregarAristaAMST(String origen, String destino, double peso) {
        String key = origen.compareTo(destino) < 0 ? origen + "-" + destino : destino + "-" + origen;
        aristasEnMST.add(key);
        pesoTotal += peso;
        repaint();
    ...
    protected void paintComponent(Graphics g) {
            ...
            for (NodoAdy adyacente = origen.getSiguiente(); adyacente != null; adyacente = adyacente.getSiguiente()) {
                ...
                if (posOrigen != null && posDestino != null) {
                    ...
                    if (soloEvaluadas) {
                        ...
                        dibujar = aristasEnMST.contains(key);
                    } else {
                        dibujar = true;
                    }
     ...
     private void dibujarNodos(Graphics2D g2d) {
        ...
        for (Map.Entry<String, Point> entry : posiciones.entrySet()) {
            ...
            Color colorNodo;
            if (nombre.equals(ciudadInicial)) {
                colorNodo = new Color(144, 238, 144); // Verde claro para el nodo inicial
            } else if (nodosEnMST.contains(nombre)) {
                colorNodo = new Color(241, 192, 192); // Rojo claro para nodos en MST
            } else {
    ...
    /**
     * Metodo para mostrar la informacion en pantalla durante la ejecucion del algoritmo
     * @param g2d 
     */
    private void dibujarInformacion(Graphics2D g2d) {
        ...
        g2d.drawString("Nodos en MST: " + nodosEnMST.size(), 20, 50);
        g2d.drawString("Aristas en MST: " + aristasEnMST.size(), 20, 70);
        g2d.drawString("Peso total: " + (int) pesoTotal + " km", 20, 90);
        g2d.drawString("Nodo inicial: " + ciudadInicial, 20, 110);
    }
